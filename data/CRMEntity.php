...
class CRMEntity {
        ...
        if(count($_FILES)) {
            ...
            foreach ($_FILES as $file) {
                if (!empty($file['name'])) {
                    ...
                    if (!in_array($fileExt, $GLOBALS['validationConfig']['allow_uploaded_exts'])) {
    ...
    function retrieve_entity_info($record, $module, $allowDeleted = false) {
        ...
        if ($cachedModuleFields) {
            ...
            // Modified by Hieu Nguyen on 2020-05-25 to fix bug Vtiger_Record_Model::getInstanceById('<Id>', '<Module>') return record event when the record is not matching with specified <Module>
            $where_clause .= " vtiger_crmentity.setype = ? AND vtiger_crmentity.crmid = ?";
            $moduleName = ($module == 'Events') ? 'Calendar' : $module;
            $params[] = $moduleName;
            $params[] = $record;
            // End Hieu Nguyen
