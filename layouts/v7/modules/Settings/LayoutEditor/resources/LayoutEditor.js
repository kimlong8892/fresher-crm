...
Vtiger.Class('Settings_LayoutEditor_Js', {
	...
	showFieldEditModel: function (data, blockId, fieldContainer) {
		...
		// Added by Hieu Nguyen on 2018-10-01
		data.find('select[name="fieldType"]').on('change', function(e) {
			...
			var generatedFieldName = '';

			// Append prefix as module name for Picklist field name
			if(value == 'Picklist' || value == 'MultiSelectCombo') { // Added MultiSelectCombo by Phu Vo on 2019.07.17
				var sourceModule = jQuery('#selectedModuleName').val();
				generatedFieldName = sourceModule.toLowerCase() + '_';
			}

			var fieldName = data.find('input[name="fieldName"]');
			fieldName.val(generatedFieldName);
			fieldName.data('value', generatedFieldName);
		});
		...
		// Added by Hieu Nguyen on 2018-10-01
		data.find('input[name="fieldName"]').on('change', function(e) {
			var value = jQuery(e.currentTarget).val().trim();
					...
					submitHandler: function (form) {
						...
						thisInstance.saveBlockDetails(form).then(
							function (data) {
								...
								if (data) {
									blockElement.find('.translatedBlockLabel').text(blockLabelValue);
...
jQuery(function($) {
	...
	$('.addBlockModal').find('input[name="labelDisplayEn"]').on('change', function (e) {
