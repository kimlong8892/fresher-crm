    ...
    <table class = "table table-borderless" cellpadding = "30" >
        <span>
			...
			{else}
                ...
                <!-- Add By Kelvin Thang -- Date: 2018-06-29-->
                {if $HAS_FILE_TEMPLATE}
                    <span>
                        {'LBL_DOWN_LOAD_TEMPLATE_XLSX_FILE'|@vtranslate:$MODULE}
                        <a style=" color: #15c !important; " href="index.php?module=Documents&action=DownloadFile&record=1&fileid=1&module_template={$DOWN_FILE_TEMPLATE_MODULE_NAME}&file=template" title="Download File">{'LBL_CLICK_HERE_XLSX_FILE'|@vtranslate:$MODULE}</a>
                    </span>
                {/if}
			{/if}
        </span>
        ...
        <!-- Modified By Kelvin Thang -- Date: 2018-06-28-->
        {if $FORMAT eq 'csv' || $FORMAT eq 'xlsx' || $FORMAT eq 'xls'}
        ...
        <!-- Modified By Kelvin Thang -- Date: 2018-06-28-->
        {if $FORMAT eq 'csv' || $FORMAT eq 'xlsx' || $FORMAT eq 'xls'}
            <tr id="delimiter_container" style="height:50px; {if $FORMAT eq 'xlsx' || $FORMAT eq 'xls'} display: none; {/if}">
