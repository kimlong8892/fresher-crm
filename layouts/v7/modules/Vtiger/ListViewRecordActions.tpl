...
<div class="table-actions" style="width: 160px;">
    ...
    {if $RECORD_ACTIONS}
        {if $RECORD_ACTIONS['edit'] && $LISTVIEW_ENTRY->isEditable()} {* Modified by Phu Vo on 2019.09.16 to show/hide edit button base on record editable *}
            <span>
                <a class="fa icon edit fa-edit" data-id="{$LISTVIEW_ENTRY->getId()}" href="{$LISTVIEW_ENTRY->getEditViewUrl()}&app={$SELECTED_MENU_CATEGORY}" name="editlink" title="{vtranslate('LBL_EDIT', $MODULE)}"></a>
            </span>
        {/if}
	{/if}
