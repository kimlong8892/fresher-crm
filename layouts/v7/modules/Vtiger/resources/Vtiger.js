...
vtUtils.openQuickCreateModal = function(moduleName, params = {}) { // Modified by Phu Vo on 2018.02.25 => add params to quickCreateForm
	...
	// Modified by Phu Vo on 2019.12.31, from the Calendar v2, quick create with parent Contact
	// work the same way with every module else
...
vtUtils.openRelatedListModal = function(params) {
    ...
    let getRelatedListSearchParams = function(container) {
        ...
        relatedListTable.find('.listSearchContributor').each(function(index,domElement){
            let searchInfo = [];
            let searchContributorElement = jQuery(domElement);
    ...
    let registerEvents = function(container) {
        ...
        // Prevent modal form submition
        container.find('form').on('submit', () => false);
    }

    let showScroll = function(container) {
        let scrollContainer = container.find('.bottomscroll-div');

        scrollContainer.css({
            'height': '400px'
        });

        app.helper.showBothAxisScroll(scrollContainer);
    }
    ...
    app.request.post({data: {...initParams, ...params.data}}).then((err, res) => {
        app.helper.hideProgress();
        if(err === null) {
            let modal = $('.modal-template-lg').clone().addClass('relatedListModal');

            let options = {
                preShowCb: modal => {
                    // Add fancy scrollbar 
                    modal.find('.relatedContents').toggleClass('fancyScrollbar', true);
                    // Process modal title
