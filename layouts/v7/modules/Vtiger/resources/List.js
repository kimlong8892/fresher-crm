...
//			http://stackoverflow.com/questions/26482645/number-isintegerx-which-is-created-can-not-work-in-ie
	...
	registerListViewBasicActions: function () {
		...
		app.event.on('post.listViewMassAction.loaded', function (e, container) {
			...
			if ($('#massSave').find('[name=module]').val() === 'SMSNotifier') {
...
//      
	...
	registerFloatingThead: function () {
		...
		// Added by Phuc on 2020.03.18 to calculate max height for listview table
		var intHeight = parseInt(height);
		if (window.innerHeight - jQuery('.table-container').offset()['top'] - 30 < intHeight) {
			height = (intHeight - 10) + 'px';
		}
		// Ended by Phuc
