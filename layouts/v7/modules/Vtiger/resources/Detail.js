...
Vtiger.Class("Vtiger_Detail_Js",{
		...
		app.helper.checkServerConfig(module).then(function(data) {
			if (data == true) {
				var cb = function(container) {
					...
					// SMSNotifierHelper.initSMSLimitValidator();
					// SMSNotifierHelper.initSMSTemplateSelector();
					// End Phu Vo
				}
	...
	registerSendSmsSubmitEvent: function() {
		...
		jQuery('body').on('submit', '#massSave', function(e) {
			...
			if (!SMSNotifierHelper.validateSMSOnlyASCII($(this).find('[name="message"]')[0])) {
				return false;
			}
			// End Feature #89
	...
	SendSmsSave: function(form) {
		...
		app.request.post({data: formData}).then(
				function(err, data) {
					...
					if(err){
						// Modified by Hieu Nguyen on 2018-07-09 to fix bug translation
						app.helper.showErrorNotification({message:app.vtranslate('JS_FAILED_TO_SEND_SMS')});
						return;
					}
					...
					if(status == 'Failed') {
						// Modified by Hieu Nguyen on 2018-07-09 to fix bug translation
						var errorMsg = app.vtranslate('JS_FAILED_TO_SEND_SMS') + '<br/>' + statusDetails.statusmessage;
						app.helper.showErrorNotification({ 'title': app.vtranslate('JS_FAILED'), 'message': errorMsg });
					} else {
						// Modified by Hieu Nguyen on 2018-07-09 to fix bug translation
						var msg = app.vtranslate('JS_SENT_SMS_SUCCESSFULLY');
	...
	showOverlayEditView: function(recordUrl) {
		...
		app.request.get({data: params}).then(function(err, response) {
				...
				var height = jQuery(window).height() - jQuery('.app-fixed-navbar').height() - jQuery('.overlayFooter').height() - 80;
					...
					// Comment out by Phu Vo on 2019.06.17 => Work around scroll with validate message issue
					// app.helper.showVerticalScroll(jQuery('.editViewContents'), scrollParams);
					// End Work around scroll with validate message issue
