...
Vtiger_Edit_Js("Calendar_Edit_Js",{
	...
	openPopUp : function(e){
		...
		var sourceFieldElement = parentElem.find('input.sourceField');  // Modified by Hieu Nguyen on 2020-03-24 to get the right source field for new contact invitees field DOM
	...
	registerRelatedContactSpecificEvents : function(form) {
        ...
        /* Commented out these lines by Hieu Nguyen on 2020-03-24 to use the default logic for Contacts invitees lookup popup
		...
		this.getRelatedContactElement(form).select2({
			 ...
			 ajax : {
				...
				'data' : function(term,page){
					 ...
					 /* Commented out these lines as this field is now understood as Contact invitee, not linked to any parent module any more
                     var parentIdElement  = form.find('[name="parent_id"]');
		...
		form.find('[name="contact_invitees"]').on(Vtiger_Edit_Js.refrenceMultiSelectionEvent,function(e,result){
    ...
    /* Commented out by Hieu Nguyen on 2020-03-24 to use the default logic from parent class
	...
	fillRelatedContacts : function(form) {
        ...
        // Modified by Hieu Nguyen on 2020-03-24 to fill selected contact invitees
		var selectedContactInvitees = form.find('[name="contact_invitees"]').data('selectedTags');
        
        if (selectedContactInvitees) {
            this.getRelatedContactElement(form).select2('data', selectedContactInvitees);
        }
        // End Hieu Nguyen
	},
	...
	addNewContactToRelatedList : function(newContactInfo, form){
		if(form.length <= 0) {
		 ...
		 var resultentData = new Array();
            ...
            /* Commented out these lines by Hieu Nguyen on 2020-03-24 as this logic is not used anymore
	...
	referenceCreateHandler : function(container) {
		...
		if(container.find('input.sourceField').attr('name') != 'contact_invitees'){ // Modified by Hieu Nguyen on 2020-03-24 to fix contact invitees quick create error
