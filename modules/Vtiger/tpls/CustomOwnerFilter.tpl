{strip}
    ...
    {if !isset($OPTIONAL_FILTER)} {assign var=OPTIONAL_FILTER value=true} {/if}
    ...
    <div class="user-list">
        {if $OPTIONAL_FILTER} {* Incase filter have optional filter (Dashboard Widget) *}
            <div class="user-list_optional">
                {* Modified by Phu Vo on 2019.09.27 to change default filter to mine*}
        ...
        <div class="user-list_input">
            <input type="text" 
                ...
                {if $VALIDATE} data-rule-required="true" data-rule-main-owner="true" {/if}
            />
        </div>
    </div>
{/strip}