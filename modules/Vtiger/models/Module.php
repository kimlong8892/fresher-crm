	...
	public static function getAll($presence = array(), $restrictedModulesList = array()) {
		...
		if(!$moduleModels){
			...
			for($i=0; $i<$noOfModules; ++$i) {
                ...
                if (isForbiddenFeature($row['name'])) {
                    continue;
                }
                // End Hieu Nguyen
...
//						3 => 'LBL_PREFER_VTIGER_RECORD',
	...
	public function getModuleIcon() {
		...
		/*Modified by Kelvin Thang -- Date: 2018-07-03*/
		$moduleIcon = "<i class='vicon-$lowerModuleName' title='$title' style='font-size: 18px;'></i>";
		if ($this->source == 'custom') {
			$moduleShortName = mb_substr(trim($title), 0, 2);
			$moduleIcon = "<span class='custom-module' title='$title'>$moduleShortName</span>";

            /*Added by Kelvin Thang -- Date: 2018-08-08*/
            $moduleIcon = "<i class='vicon-documents' title='$title' style='font-size: 18px;'></i>";
            $imageFilePath = "resources/images/modules/{$moduleName}.png";

            if (file_exists($imageFilePath)) {
                $moduleIcon = "<img src='$imageFilePath' title='$title' style='font-size: 18px; height: 18px; width: 18px;'/>";
            }
		}

		$imageFilePath = 'layouts/'.Vtiger_Viewer::getLayoutName()."/modules/$moduleName/$moduleName.png";
		if (file_exists($imageFilePath)) {
			$moduleIcon = "<img src='$imageFilePath' style='font-size: 18px; height: 18px; width: 18px;' title='$title'/>";
		}
