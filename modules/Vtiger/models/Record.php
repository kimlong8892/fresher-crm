...
class Vtiger_Record_Model extends Vtiger_Base_Model {
    ...
    public static function getInstanceByConditions($moduleName, array $conditions = []) {
        ...
        $sql = "SELECT {$moduleModel->basetableid} FROM {$moduleModel->basetable} WHERE 1 = 1 ";
        ...
        if(!empty($matchedId)) {
            return self::getInstanceById($matchedId, $moduleName);
        }
	...
	public static function getSearchResult($searchKey, $module=false) {
		...
		// Added by Dung Bui on 2020.06.09 search mobile, phone on module Contacts
        if ($module == 'Contacts') {
            $query = "SELECT label, crmid, setype, createdtime
              FROM vtiger_crmentity
              INNER JOIN vtiger_contactdetails ON (contactid = crmid)
              WHERE (label LIKE ? OR phone LIKE '%".$searchKey."%' OR mobile LIKE '%".$searchKey."%') AND vtiger_crmentity.deleted = 0";
        }
        // End by Dung Bui
