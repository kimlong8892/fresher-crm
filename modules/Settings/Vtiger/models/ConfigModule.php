...
class Settings_Vtiger_ConfigModule_Model extends Settings_Vtiger_Module_Model {
	...
	public function save() {
		...
		// update instance help desk on PMS
		self::sendApiUpdateInstanceHelpDesk($updatedFields);
     ...
     * Author: Dung Bui
     * Date: 2020.04.13
     * Function to send Api update instance helpdesk info on PMS
     * @request $fields (helpdesk) array
     */
    private static function sendApiUpdateInstanceHelpDesk($fields) {
        global $urlApiSetupNewInstance, $deploymentApiInfo, $site_URL;

        $log = LoggerManager::getLogger('UPDATE_INSTANCE_HELP_DESK');
        $log->info('[CRON] UPDATE INSTANCE HELP DESK - Start Setup');

        try {
            $paramsLogin = array(
                "RequestAction" => "Login",
                "Credentials" => array(
                    'username' => $deploymentApiInfo['username'],
                    'access_key' => $deploymentApiInfo['access_key'],
                    'user_type' => 'api',
                ),
            );

            $loginResponse = Vtiger_SetupInstance_Model::callDeploymentApi($urlApiSetupNewInstance, $paramsLogin, '');
        }
        catch (Exception $ex) {
            $log->error('[CRON] UPDATE INSTANCE HELP DESK - Call Api: Login' . $ex->getMessage());
        }
        ...
        try {
            $paramsUpdateInstanceHelpDesk = array(
                "RequestAction" => "UpdateInstanceHelpDesk",
                "Data" => array(
                    'url' => $site_URL,
                    'helpdesk_name' => $fields['HELPDESK_SUPPORT_NAME'],
                    'helpdesk_email' => $fields['HELPDESK_SUPPORT_EMAIL_ID'],
                ),
            );

            Vtiger_SetupInstance_Model::callDeploymentApi($urlApiSetupNewInstance, $paramsUpdateInstanceHelpDesk, $token);
        }
        catch (Exception $ex) {
            $log->error('[CRON] SETUP  UPDATE INSTANCE HELP DESK - Call Api: UpdateInstanceHelpDesk' . $ex->getMessage());
        }

        $log->info('[CRON] UPDATE INSTANCE HELP DESK - End Setup');
