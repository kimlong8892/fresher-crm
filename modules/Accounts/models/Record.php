...
class Accounts_Record_Model extends Vtiger_Record_Model {
    ...
    static function getReportSumaryAccount($startDate, $endDate){
        global $adb;
        ...
        $sql = "SELECT vtiger_products.serialno,
                ...
                INNER JOIN vtiger_account a ON (vtiger_salesorder.accountid = a.accountid)
                INNER JOIN vtiger_crmentity as v_account ON (v_account.crmid = a.accountid)
                WHERE DATE(v_order.createdtime) >= ?
                AND DATE(v_order.createdtime) <= ?
                GROUP BY vi.productid, salesorderid";
        ...
        while ($row = $adb->fetchByAssoc($result)) {
            ...
            if ($accounts[$row['accountid']]['id'] == $row['accountid']) {
                $accounts[$row['accountid']]['products'][] = $row;
            } else {
                $accounts[$row['accountid']]['id'] = $row['accountid'];
                $accounts[$row['accountid']]['name'] = $row['accountname'];
                $accounts[$row['accountid']]['products'][] = $row;
            }
            $return[] = $row;
        }
