...
var Vtiger_PBXManager_Js = {
    ...
    registerPBXOutboundCall: function(number, record, callLogId) {
        ...
        Vtiger_PBXManager_Js.makeOutboundCall(number, record, callLogId);
    },
    /**
        ...
        app.request.get({ url: url }).then(function(e, result) {
            if (result) {
                for (i = 0; i < result.length; i++) {
                    var record = result[i];
                    if (
                        jQuery("#pbxcall_" + record.pbxmanagerid + "").size() ==
                        0
                    )
    ...
    showPBXIncomingCallPopup: function(record) {
        var params = {
            ...
            message:
                '<div class="row-fluid pbxcall" id="pbxcall_' +
                record.pbxmanagerid +
                '" callid=' +
                record.pbxmanagerid +
                ' style="color:black"><span class="span12" id="caller" value="' +
                record.customernumber +
                '">' +
                app.vtranslate("JS_PBX_CALL_FROM") +
                " : " +
                record.customernumber +
                '</span><span class="hide span12" id="contactsave_' +
                record.pbxmanagerid +
                '">\n\
					<span><input class="span3" id="email_' +
                record.pbxmanagerid +
                '" type="text" placeholder="Enter Email-id"></input>&nbsp;&nbsp;&nbsp;<select class="input-small" id="module_' +
                record.pbxmanagerid +
                '" placeholder="Select"><option>Select</option></select><h5 class="alert-danger hide span3" id="alert_msg">' +
                app.vtranslate("JS_PBX_FILL_ALL_FIELDS") +
                '</h5>\n\
					<button class="btn btn-success pull-right" id="pbxcontactsave_' +
                record.pbxmanagerid +
                '" recordid="' +
                record.pbxmanagerid +
        ...
        if (record.answeredby != null) {
            jQuery(
                "#answeredbyname",
                "#pbxcall_" + record.pbxmanagerid + ""
            ).text(record.answeredby);
            jQuery(
                "#answeredby",
                "#pbxcall_" + record.pbxmanagerid + ""
            ).show();
        ...
        jQuery("#pbxcontactsave_" + record.pbxmanagerid + "").bind(
            "click",
            function(e) {
                ...
                jQuery("#pbxcontactsave_" + record.pbxmanagerid + "").unbind(
                    "click"
                );
            }
        );
    ...
    createRecord: function(e, record) {
        ...
        var number = jQuery("#caller", "#pbxcall_" + pbxmanagerid + "").attr(
            "value"
        );
        var url =
            "index.php?module=PBXManager&action=IncomingCallPoll&mode=createRecord&number=" +
        ...
        switch (record.callername) {
            case null:
                ...
                app.request.get({ url: url }).then(function(e, result) {
                    ...
                    if (showSaveOption)
                        jQuery(
                            "#contactsave_" + record.pbxmanagerid + ""
                        ).show();
                });
                break;
            default:
                jQuery("#caller", "#pbxcall_" + record.pbxmanagerid + "").html(
                    app.vtranslate("JS_PBX_CALL_FROM") +
    ...
    removeCompletedCallPopup: function() {
        ...
        for (var i = 0; i < pbxcall.length; i++) {
            ...
            var url =
                "index.php?module=PBXManager&action=IncomingCallPoll&mode=getCallStatus&callid=" +
    ...
    removeCallPopup: function(callid) {
        jQuery("#pbxcall_" + callid + "")
            .closest(".vt-notification")
            .remove();
    },
    ...
    getContentHolder: function(view) {
        if (view == "List") return jQuery(".listViewContentDiv");
        else return jQuery(".detailViewContainer");
    },

    // Added by Hieu Nguyen on 2020-02-18 to handle make call with phone selector
    makeCallWithPhoneSelector: function (customerId, customerName, phoneNumbers, callId) {
        // Make call right away if the customer has only 1 phone number
        if (phoneNumbers.length == 1) {
            Vtiger_PBXManager_Js.registerPBXOutboundCall(phoneNumbers[0].number, customerId, callId);
        }
        // Show phone number selector if the customer has more than 1 phone number
            ...
            phoneNumbers.forEach((phoneNumber) => {
                var phoneClickLogic = `Vtiger_PBXManager_Js.registerPBXOutboundCall('${phoneNumber.number}', '${customerId}', '${callId}'); app.helper.hideModal();`;
                var row = `<tr>
                        <td>${phoneNumber.field_label}</td>
                        <td>${phoneNumber.number}</td>
                        <td class="text-center">
                            <a href="javascript:void(0);" onclick="${phoneClickLogic}"><i class="fa fa-2x fa-phone"></i></a>
                        </td>
                    </tr>`;
     ...
     */
    makeOutboundCall: function(number, record, callLogId) { // Modified param by Hieu Nguyen on 2019-12-31 to pass the call log id to update
        var params = {
            number: number,
            record: record,
        ...
        app.request.post({ data: params }).then(function(e, result) {
            ...
            if (e) {
                return app.helper.showErrorNotification({message: e.message});
            }
            // End Phu Vo

            if (result == true || result.result == true) {
                // Fixed by Hieu Nguyen on 2018-10-08
                params = {
    ...
    registerEvents: function() {
        ...
        //for polling
        var url =
            "index.php?module=PBXManager&action=IncomingCallPoll&mode=checkPermissionForPolling";
        app.request.get({ url: url }).then(function(e, result) {
            if (result) {
...
jQuery(window).load(function() {
    // Comment out by Phu Vo on 2018.12.13 => Use OnlineCRM Call Popup instead
